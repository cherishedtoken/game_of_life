...
int main() {
    ...
    for (int i = 0; i < GAMEBOARD_SIZE; i++) {
        for (int j = 0; j < GAMEBOARD_SIZE; j++) {
            gameboard_history[i][j] = gameboard[i][j];
        }
    }
        ...
        for (int i = 0; i < GAMEBOARD_SIZE; i++) {
            for (int j = 0; j < GAMEBOARD_SIZE; j++) {
                if (gameboard_history[i][j] >= STABLE_CYCLES) {
                    gameboard_copy[i][j] = 0;
                } else if (gameboard_history[i][j] <= -STABLE_CYCLES) {
                    gameboard_copy[i][j] = 1;
                } else if (gameboard_copy[i][j] == 1 &&
                    gameboard_history[i][j] >= gameboard_copy[i][j]) {
